...
//        .toLocalTime();
    ...
    public static List<UserMealWithExceed> getFilteredWithExceeded(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> totalCaloriesByDate = mealList.stream().collect(Collectors.toMap(userMeal ->
        ...
        return mealList.stream().filter(meal -> TimeUtil.isBetween(meal.getDateTime().toLocalTime(), startTime, endTime))
                .map(meal -> new UserMealWithExceed(meal, totalCaloriesByDate.get(meal.getDateTime().toLocalDate()) > caloriesPerDay))
